...
/** Code sample for staking hook demo */
const CODE_SAMPLE = `import { useCallback, useState } from "react";
    ...
    <div className="container py-24 md:py-32 text-neutral-300 font-[geist-sans] antialiased px-4">
      ...
      >
        ...
        >
          ...
          blockchain precision with human-centric design. From contract logic to
          front-end nuance, every layer is crafted with intent. Secure,
      ...
      >
        ...
        <p className="text-neutral-500 font-light leading-relaxed max-w-2xl mx-auto">
          Each build follows a verifiable, modular pattern, from smart contract
          to UI. Below, a simplified staking flow represents our mindset:
          clarity, safety, intent.
        </p>
      ...
      {/* ─── UI Snapshot ───────────────────────────── */}
      <section aria-labelledby="dapp-snapshot-title" className="mt-12 text-center">
        <motion.div
          initial={{ opacity: 0, y: 20 }}
          whileInView={{ opacity: 1, y: 0 }}
          transition={{ duration: 0.8, ease: "easeOut" }}
        ...
        >
          ...
          <div className="space-y-3">
            <label htmlFor="stake-amount" className="text-sm text-neutral-400">
              Amount to Stake
            </label>
            <input
              id="stake-amount"
          ...
          <p className="text-xs text-neutral-500 pt-2">
            Example UI built with{" "}
            <span className="text-teal-400">shadcn/ui</span> +{" "}
            <span className="text-teal-400">wagmi</span> — precise, predictable,
            intentional.
          </p>
        </motion.div>
      </section>
