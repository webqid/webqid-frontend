...
const cards = [
  {
    ...
    body:
      "We translate visual systems into performant, precise frontends. Shared Figma-to-code workflows, pixel-accurate delivery.",
  },
  {
    title: "Motion Artists",
    body:
      "Subtle transitions, cinematic flow, crafted with Motion or GSAP. Movement that feels inevitable, never loud.",
  },
  {
    title: "Creative Developers",
    body:
      "Open-source tools, experimental prototypes, or shared R&D. We enjoy pushing boundaries quietly and precisely.",
  },
];

function Sep(props: { className?: string }) {
  return <Separator className={`my-24 bg-neutral-800/70 ${props.className ?? ""}`} />;
}
  ...
  return (
    <div className="container py-28 md:py-36 text-neutral-300 font-[geist-sans] antialiased">
      ...
      <section className="text-center space-y-6">
        <motion.h1 {...baseMotion} className="text-5xl md:text-6xl font-medium tracking-tight after:content-['.'] after:text-teal-500 after:ml-1 after:font-[geist-mono]">
          collaborations
        </motion.h1>

        <motion.p
          {...baseMotion}
          transition={{ ...baseMotion.transition, delay: 0.1 }}
      ...
      <section className="max-w-5xl mx-auto space-y-12">
        ...
        <motion.div {...inViewMotion} className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6 mt-16">
          {cards.map(({ title, body }) => (
            ...
            >
              <h3 className="text-neutral-100 text-lg font-medium tracking-tight">{title}</h3>
              <p>{body}</p>
            </div>
          ))}
        </motion.div>
      ...
      <section className="max-w-3xl mx-auto text-center space-y-8">
        <motion.p {...inViewMotion} className="text-neutral-400 font-light leading-relaxed">
      ...
      {/* ─── Call To Connect ───────────────────────────── */}
      <motion.div {...inViewMotion} className="text-center space-y-4 font-sans">
